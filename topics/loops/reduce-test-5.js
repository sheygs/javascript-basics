//grab all the text from a webpage
...
reduce	Full support3	Full support12	Full support3	Full support9	Full support10.5	Full support4.1	Full support≤37	Full support18	Full support4	Full support14	Full support4	Full support1.0	Full supportYes
...
Metadata
Last modified: Feb 18, 2020, by MDN contributors
Related Topics
...
function alphaNumericChars(string){
  return string.match(/[a-z0-9]/ig);
}

function lowercaseChars(chars){
  return chars.toLowerCase();
}

function letterCounter(acc,char){
  acc[char] ? acc[char]++ : acc[char] = 1;
  return acc;
}

function sortByHighest(curr, next){
    const [,value1] = curr;
    const [,value2] = next;
    return value2 - value1;
}

const allowedCharacters = alphaNumericChars(text)
                              .map(lowercaseChars)
                              .reduce(letterCounter,{});
                              
const sorted = Object.entries(allowedCharacters)
                       .sort(sortByHighest);
